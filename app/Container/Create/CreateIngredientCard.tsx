...
const CreateIngredientCard = (props: createIngredientCardProps) => {
    ...
    const clearIcon = input ?
        <Icon onPress={() => setInput('')} type={'feather'} name={'x'} onPressIn={() => setInput('')} color={Theme.Light.caption} />
        : null;

    return (
        <View style={createIngredientCardStyle.container}>
            <TextInput placeholder={'Ingredient'} value={input} onChangeText={(text: string) => setInput(text)} style={createIngredientCardStyle.input} onSubmitEditing={() => console.log('submitted')} />
            {clearIcon}
        </View>
    );
};
